...
  for(q=p;q->Parent;q=q->Parent)   //判断q所指向的节点，左孩子置0，右孩子置1 
	   if(q==q->Parent->LChild)     
		...
		HC[i].code[--HC[i].start]='1';   
            ...
            FILE *fp; 
            if((fp=fopen("对应编码.txt","w+"))==NULL)  {   printf("无法打开\n");   exit(1);  } 
                    for(i=0;i<n;i++) 
...
        do  
        {   clearscreen(); 
	            printf("赫夫曼编码译码系统\n");   printf("1.进行编码\n");   printf("2.进行译码\n");   printf("0.退出\n");   scanf("%c",&choice);   getchar();   switch(choice)   { 
